    ...
    ctx = {
        ...
        "ндс_предложение": d.get("ндс_предложение")
            or ("НДС не облагается, поскольку Подрядчик является плательщиком налога "
                ...
                f"Сумма облагается НДС по ставке {num_str(ндс_ставка)}%, в том числе "
                f"НДС {num_str(ндс_ставка)}% — {money_fmt(ндс_сумма)} руб." if ндс_ставка
                else ("Исполнитель освобожден от уплаты НДС в соответствии с п. 1 "
                      "ст. 145 НК РФ. В случае утери Исполнителем права на "
                      "освобождения от уплаты НДС сумма, указанная в настоящем счете, "
                      "не подлежит изменению, НДС подлежит выделению из указанной в "
                      "счете суммы.")),
    }
